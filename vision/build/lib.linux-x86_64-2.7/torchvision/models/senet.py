...
def initialize_pretrained_model(model, num_classes, settings):
    ...
    pretrained_dict = torch.load('./checkpoints/senet154-c7b49a05.pth')
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)

def senet154(pretrained=False, params=None):
    pretrained = 'imagenet'
    model = SENet(SEBottleneck, [3, 8, 36, 3], groups=64, reduction=16,
                  dropout_p=0.2, num_classes=params['num_classes'])
    loss_model = LRCP_LOSS(params)
    if pretrained is not None:
        settings = pretrained_settings['senet154'][pretrained]
        initialize_pretrained_model(model, params['num_classes'], settings)
